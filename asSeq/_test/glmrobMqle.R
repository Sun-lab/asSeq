...
#### based originally on Eva Cantonis S-plus code "robGLM"
...
## Estimated asymptotic covariance of the robust estimator
  ...
  if(any(ina)) {# put NAs back, extending theta[] to "original length"
    ok <- !ina
    theta.na[ok] <- theta ; theta <- theta.na
## also extend the "p x p" matrices with NAs --
...
## MM: 'acc' seems a misnomer to me, but it is inherited from  MASS::rlm
glmrobMqle.control <-
...
##
##   FIXME(2): Some of these fail when Huber "c", 'tcc' is = +Inf
##   -----    --> ../../robGLM1/R/rglm.R
