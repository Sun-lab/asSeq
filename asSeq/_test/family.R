...
binomial <- function (link = "logit")
{
  ...
  initialize  = expression({
                            if (NCOL(y) == 1) {
                              ...
                              ## anything, e.g. NA/NaN, for cases with 
                              ## zero weight is OK.
                            ...
                            else stop("for the binomial family, y must be a vector 
                                     of 0 and 1\'s\n",
...
quasibinomial <- function (link = "logit")
{
  ...
  initialize = expression({
                          ...
                          else stop("for the quasibinomial family, 
...
inverse.gaussian <- function(link = "1/mu^2")
{
  ...
  validmu <- function(mu) TRUE
  simfun <- function(object, nsim) {
    if(is.null(tryCatch(loadNamespace("SuppDists"), error <- function(e) NULL)))
