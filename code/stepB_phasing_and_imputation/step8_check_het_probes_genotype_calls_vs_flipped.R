# Checking the pipeline input genotype calls vs. the fliped output
...
flipped = "../../data/chr_flipped"
sample_table = read.table(sprintf("%s/geno.chr22_flip.ped", flipped), as.is=TRUE)
...
haps_table = foreach(i=1:22, .combine=rbind) %dopar% {
  pedfile = read.table(sprintf("%s/geno.chr%s_flip.ped", flipped, i), as.is=TRUE)
  mapfile = read.table(sprintf("%s/geno.chr%s_flip.map", flipped, i), as.is=TRUE)
  if (i == 1) {
