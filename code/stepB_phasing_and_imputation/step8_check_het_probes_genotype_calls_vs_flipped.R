# Checking the pipeline input genotype_calls.txt vs. the flipped output
...
flipped = "/fh/fast/sun_w/licai/eQTL_KIRC/data/chr_flipped"
sample_table = read.table(sprintf("%s/KIRC.chr22_flip.ped", flipped), as.is=TRUE)
...
haps_table = foreach(i=1:22, .combine=rbind) %dopar% {
  pedfile = read.table(sprintf("%s/KIRC.chr%s_flip.ped", flipped, i), as.is=TRUE)
  mapfile = read.table(sprintf("%s/KIRC.chr%s_flip.map", flipped, i), as.is=TRUE)
  if (i == 1) {
    print(dim(mapfile[,c(1, 2, 4)]))
    print(mapfile[1:5, c(1, 2, 4)])
    print(dim(pedfile[sample_index, seq(7, ncol(pedfile)-1, by=2)]))
    print(dim(pedfile[sample_index, seq(8, ncol(pedfile), by=2)]))
    print(pedfile[sample_index, 1:16])
  }
  cbind(mapfile[,c(1, 2, 4)],
        t(pedfile[sample_index, seq(7, ncol(pedfile)-1, by=2)]),
        t(pedfile[sample_index, seq(8, ncol(pedfile), by=2)]))
}
stopCluster(cl)

#haps_table = do.call("rbind", haps_table_list)
colnames(haps_table) = c("Chromosome", "dbSNP.RS.ID", "Physical.Position", "genotype1", "genotype2")
head(haps_table)
...
table(genotype_merged[,sample_cel_file] , 
      (genotype_merged$genotype1 == genotype_merged$genotype2))
