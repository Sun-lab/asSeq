...
trecase2 <- function(Y, Y1, Y2, Z, X, SNPinfo, Geneinfo, fam_nb, file_out,
  ...
  for(gg in 1:nrow(Geneinfo)){
    ...
    y2 = Y2[, gg]    
    lgy1 = Rcpp_lgy_add_1(y)
    reg0 = Rcpp_reg_BFGS(y, X, rep(0,nrow(XX)), rep(0,ncol(XX)+1),
                         fam_nb = T, lgy1, max_iter, eps, show) 

    gene_snp_res = NULL
    
    for(ss in ssChr:nrow(SNPinfo)){
      # ss = 1
      if(SNPinfo[ss,2] != Geneinfo[gg,2]) break
      ...
      if(SNPinfo[ss,3] > Gene_start[gg] & SNPinfo[ss,3] < Gene_end[gg]){
        ...
        res_trec = Rcpp_trec(y, X, zz, fam_nb, lgy1, ini_bxj, reg0$LL, 
                             reg0$PAR,  max_iter, eps, show)
