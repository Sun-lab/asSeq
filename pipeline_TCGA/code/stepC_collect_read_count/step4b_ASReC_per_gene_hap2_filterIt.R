...
# sbatch --mem=32000 R CMD BATCH --no-save --no-restore step4b_ASReC_per_gene_hap2_filterIt.R
...
proj_output = "../output/step4b_ASReC_per_gene_hap2_filterIt"

if (!dir.exists(proj_output)) dir.create(proj_output)
setwd(proj_input)

filenames = list.files(pattern="_filterIt_hap2.bam$")
...
setwd(pipeline_dir)
write.table(as1, file = sprintf("%s/gene_level_counts_filterIt_hap2.txt", proj_output), append = FALSE,
            quote = FALSE, sep = "\t", eol = "\n", row.names = TRUE,
            col.names = TRUE)

row.names(cts) = c("nMappedReads", "nFragMappedToGene")
write.table(cts, file = sprintf("%s/ASReC_liberal_filterIt_hap2.txt", proj_output), append = FALSE,
            quote = FALSE, sep = "\t", eol = "\n", row.names = TRUE,
            col.names = TRUE)
