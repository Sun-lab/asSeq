...
seedval = as.numeric(args[4])
cis_window = as.numeric(args[5])
model = args[6]
if(length(args)>6){
  paral = as.numeric(args[7])
}else{
  paral = 1e6
}
...
library(MatrixEQTL)
library(Matrix)
pref = "Brain_Nucleus_accumbens_basal_ganglia"
...
boutdir = sprintf("boutM%s", nsub)
routdir = sprintf("routM%s", nsub)
...
#    g.ini = matrix(as.numeric(unlist(g.ini)), ncol=nsub)
#    g.ini[g.ini==2] = 1
#    g.ini[g.ini==3] = 2
#    g.ini = matrix(as.numeric(g.ini), ncol=nsub)
#    rownames(g.ini) = rownames(g.in)[kp]
#    colnames(g.ini) = colnames(g.in)
...
#    SNP_file_name = sprintf("%s/SNP_%s.txt", boot.dir, suff0)
    SNP_file_name = sprintf("%s/SNP_%s.txt", int.dir, suff0)
#    SNP_file_name_pref = sprintf("%s/SNP_TRECASE_%s.txt", int.dir, suff0)

    write.table(g.ini, SNP_file_name, row.names=T, col.names=T, quote=F, sep="\t")
#    write.table(as.matrix(g.ini0), SNP_file_name_pref, row.names=T, col.names=T, quote=F, sep="\t")
  
  

    expression_file_name = sprintf("%s/GE_norm_%s_%s.dat", int.dir, model, suff0)
    exprj = read.table(expression_file_name)
#    exprj = read.table(expression_file_name, sep=",")
    exprj[,1:5,drop=F]
...
#  cvrt = SlicedData$new()
#  cvrt = cvrt$CreateFromMatrix(t(as.matrix(covar)))
...
#args = c("21", "1", "194", "100", "5e5", "long")
        com = sprintf("R CMD BATCH '--args %s %s %s %s %s %s' step3_runboot.R %s/step3_runboot_%s_%s_%s_%s_%s_%s.Rout",
                                           chri,blocki,nsub,numpoints,cis_window,model,  routdir,               
                                           chri,blocki,nsub,numpoints,cis_window,model)
        qout = sprintf("%s/step3_runboot_%s_%s_%s_%s_%s_%s.out",
                        boutdir,       chri,blocki,nsub,numpoints,cis_window,model) 
          com2 = sprintf("sbatch -p %s -N 1 -t 0%s-06:00:00 -o %s --mem=%s --wrap=\"%s\"", queue, days, qout, mem, com)        
          message(com2)
          if(blocki%%paral==0){
            system(com)
          }else{
