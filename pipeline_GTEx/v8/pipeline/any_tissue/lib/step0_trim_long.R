...
filt = data.frame(cut1gns=sum(rm1), cut2gns=sum(rm2), allrm=sum(aseo!=(ase1+ase2)), rm1cnt, rm2cnt, rm3cnt)
write.csv(filt, sprintf("%s/%s_filt.csv", diag.dir, pref), quote=F, row.names=F)
filt
