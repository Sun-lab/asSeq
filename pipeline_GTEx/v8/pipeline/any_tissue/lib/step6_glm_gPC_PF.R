...
out.dir = sprintf("BetaBin_gPC_PF")
if(!file.exitst(out.dir))dir.create(out.dir)
#setwd(wrk.dir)
...
#i = i + 1
        ...
        attempt = tryCatch({
          ...
          fit0 <- vglm(cbind(yL, nL-yL) ~ 1 + gPC1 + gPC2, family=betabinomial, data=phenoL,
              trace = FALSE, subset = nL > 0)

          ini = c(fit0@coefficients, 0, 0, 0, 0, 0); ini[2][ini[2]< -5] = -5
          fit1 <- vglm(cbind(yL, nL-yL) ~ 1 + gPC1 + gPC2 + PF1 + PF2 + PF3 + PF4 + PF5, family=betabinomial, data=phenoL,
              trace = FALSE, subset = nL > 0)

          ini = c(fit1@coefficients, 0); ini[2][ini[2]< -5] = -5
          ini[1][ini[1]< -5] = -5
          ini[1][ini[1]> 5] = 5
          fit <- vglm(cbind(yL, nL-yL) ~ 1 + gPC1 + gPC2 + PF1 + PF2 + PF3 + PF4 + PF5 + cnd, family=betabinomial, data=phenoL,
...
#          summary(fit)
        ...
        fit.bin1 <- glm(cbind(yL, nL-yL) ~ 1 + gPC1 + gPC2 + PF1 + PF2 + PF3 + PF4 + PF5, family=binomial, data=phenoL)
        fit.bin <- glm(cbind(yL, nL-yL) ~ 1 + gPC1 + gPC2 + PF1 + PF2 + PF3 + PF4 + PF5 + cnd, family=binomial, data=phenoL)
        fit.qua <- glm(cbind(yL, nL-yL) ~ 1 + gPC1 + gPC2 + PF1 + PF2 + PF3 + PF4 + PF5 + cnd, family=quasibinomial, data=phenoL)
        #fit
        #logLik(fit.bin)
        #resL1.bb[i,]
        ...
        attempt = tryCatch({
          ...
          fit0 <- vglm(cbind(yS, nS-yS) ~ 1 + gPC1 + gPC2, family=betabinomial, data=phenoS,
              trace = FALSE, subset = nS > 0)

          ini = c(fit0@coefficients, 0, 0, 0, 0, 0); ini[2][ini[2]< -5] = -5
          fit1 <- vglm(cbind(yS, nS-yS) ~ 1 + gPC1 + gPC2 + PF1 + PF2 + PF3 + PF4 + PF5, family=betabinomial, data=phenoS,
              trace = FALSE, subset = nS > 0)

          ini = c(fit1@coefficients, 0); ini[2][ini[2]< -5] = -5
          ini[1][ini[1]< -5] = -5
          ini[1][ini[1]> 5] = 5
          fit <- vglm(cbind(yS, nS-yS) ~ 1 + gPC1 + gPC2 + PF1 + PF2 + PF3 + PF4 + PF5 + cnd, family=betabinomial, data=phenoS,
        ...
        fit.bin1 <- glm(cbind(yS, nS-yS) ~ 1 + gPC1 + gPC2 + PF1 + PF2 + PF3 + PF4 + PF5, family=binomial, data=phenoS)
        fit.bin <- glm(cbind(yS, nS-yS) ~ 1 + gPC1 + gPC2 + PF1 + PF2 + PF3 + PF4 + PF5 + cnd, family=binomial, data=phenoS)
        fit.qua <- glm(cbind(yS, nS-yS) ~ 1 + gPC1 + gPC2 + PF1 + PF2 + PF3 + PF4 + PF5 + cnd, family=quasibinomial, data=phenoS)
        #fit
        #logLik(fit.bin)
        #resS1.bb[i,]
...
write.csv(resL1.bb, sprintf("%s/%s_long_vglm_ohet.csv", out.dir, mod), quote=F)
write.csv(resS1.bb, sprintf("%s/%s_short_vglm_ohet.csv", out.dir, mod), quote=F)

write.csv(resL1.qp, sprintf("%s/%s_long_glmqp_ohet.csv", out.dir, mod), quote=F)
write.csv(resS1.qp, sprintf("%s/%s_short_glmqp_ohet.csv", out.dir, mod), quote=F)

write.csv(resL1.bin, sprintf("%s/%s_long_glmbin_ohet.csv", out.dir, mod), quote=F)
write.csv(resS1.bin, sprintf("%s/%s_short_glmbin_ohet.csv", out.dir, mod), quote=F)
