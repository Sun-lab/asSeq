...
rownames(assJm[[j]])=rownames(assTm[[j]])=rownames(assAm[[j]]) = resi$gene

#based on our past ckechs the settings used for glm, version 3 (kp3) will be used
#y>=0 & y<=0.3
j = 1
for(j in 1:2){
  ...
  write.csv(outT[!is.na(outT$permp),], sprintf("%s_%s_TReC_trimmed.csv", suff), quote=F)
  write.csv(outA[!is.na(outA$permp),], sprintf("%s_%s_ASE_trimmed.csv", suff), quote=F)
