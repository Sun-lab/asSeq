...
#args = c("specifications_Artery_Aorta.txt")
#args = c("specifications_Brain_Caudate_basal_ganglia.txt")
args
...
for(cndi in 1:12){
  ...
  bout = sprintf("%s/step6_glm_marginal_%s.out", boutdir, cndi)
  rout = sprintf("%s/step6_glm_marginal_%s.Rout", routdir, cndi)
...
for(cndi in 1:3){
  rinp = sprintf("%s/step6_glm_nocov.R", rinpdir)
  bout = sprintf("%s/step6_glm_nocov_%s.out", boutdir, cndi)
  rout = sprintf("%s/step6_glm_nocov_%s.Rout", routdir, cndi)
  com = sprintf("R CMD BATCH '--args %s' %s %s",
                                   cndi, rinp, rout)
  com2 = sprintf("sbatch -p %s -t 0%s-00:00:00 -o %s --mem=%s --wrap=\"%s\"", 
                         queue, days, bout, mem, com)        
