...
resL = read.csv(sprintf("short_TReCASE_%s_%s_5e+05_long.csv", pref, nsam), as.is=T)
resS = read.csv(sprintf("short_TReCASE_%s_%s_5e+05_short.csv", pref, nsam), as.is=T)
resL[1:4,]
resS[1:4,]
...
write.csv(effL, sprintf("%s_ASE_cors_long.csv", pref), quote=F)
write.csv(effS, sprintf("%s_ASE_cors_short.csv", pref), quote=F)
