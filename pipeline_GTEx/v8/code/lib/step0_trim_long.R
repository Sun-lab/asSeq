...
cut1 = as.numeric(specs[10])#0.05
cut2 = as.numeric(specs[11])#0.01
cut3 = as.numeric(specs[12])#0.20
...
gene_anno_file = sprintf("%s/Reference/%s", bas.dir, gene_anno_file)
gene_anno_file2 = "gencode.v26.GRCh38.genes.sqlite"
gene_anno_file2 = sprintf("%s/Reference/%s", bas.dir, gene_anno_file2)
...
#trec
sumi = aggregate(trecD>0, by=list(gns$seqnames) ,sum)
rownames(sumi) = sumi[,1]
sumi = sumi[,-1]
apply(sumi, 1, summary)

#ase
sumi = aggregate(ase>0, by=list(gns$seqnames) ,sum)
rownames(sumi) = sumi[,1]
colnames(sumi) = trecNms
sumi = sumi[,-1]
apply(sumi, 1, summary)
...
kp = apply(kp, 2, sum)
table(kp)
if(sum(kp>0)>0)write.table(kp[kp>0], sprintf("%s/%s_noASE.txt", diag.dir, pref), col.names=F, row.names=T)
kpB = sum(kp>0)
  
#ase1
sumi = aggregate(ase1>0, by=list(gns$seqnames) ,sum)
rownames(sumi) = sumi[,1]
colnames(sumi) = trecNms
sumi = sumi[,-1]
apply(sumi, 1, summary)
...
kp = apply(kp, 2, sum)
table(kp)
if(sum(kp>0)>0)write.table(kp[kp>0], sprintf("%s/%s_noASE1.txt", diag.dir, pref), col.names=F, row.names=T)
kp1 = sum(kp>0)

#ase2
sumi = aggregate(ase2>0, by=list(gns$seqnames) ,sum)
rownames(sumi) = sumi[,1]
colnames(sumi) = trecNms
sumi = sumi[,-1]
apply(sumi, 1, summary)
...
kp = apply(kp, 2, sum)
table(kp)
if(sum(kp>0)>0)write.table(kp[kp>0], sprintf("%s/%s_noASE2.txt", diag.dir, pref), col.names=F, row.names=T)
kp2 = sum(kp>0)
 
#aseN
fN = colSums(aseN)/colSums(ase)
fN[is.na(fN)] = -median(fN, na.rm=T)
pdf(sprintf("%s/%s_ASEdiagn.pdf", diag.dir, pref), height=4, width=4)
  hist(fN, xlab="fraction wrong hap", main=pref, bty="n")
  leg = c(sum(fN<0 | fN>0.01), sum(kpB), sum(kp1), sum(kp2))
  leg = sprintf("%s=%s", c("hapN", "ASE", "ASE1", "ASE2"), leg)
  legend("topright", legend=leg, bty="n")
dev.off()
if(sum(fN<0)>0)write.table(fN[fN<0], sprintf("%s/%s_noASEN.txt", diag.dir, pref), col.names=F, row.names=T)


#move to only expressed
#at least 20% of the samples with at least 10 TReC counts
...
#
#
#for samples with at least 10 ASE counts
#quantify samples with more then 10% hapN
fracN = aseN/(ase1+ase2+aseN)>.1 & (ase1+ase2+aseN)>=10
frac1 = ase1/(ase1+ase2)
kp = (ase1+ase2)>10

#frac-extr, frac-n, num to use
...
for(min.samples in c(5, 10, 20)){
  pdf(sprintf("%s/%s_fracExtr_byfracWrong_%s.pdf", diag.dir, pref, min.samples), height=6, width=6)
    ...
    for(cut.hN in c(0.01, 0.05, 0.1)){
      ...
      plot(density(frac1ag[kp2a,1]), bty="n", xlim=0:1, 
        main=sprintf("%%samp. w/hapN>%s", cut.hN, xlab="frac. of samp. w/ extreme prop."))#, ylim=c(0,1)
...
frac9[is.na(frac9)] = -median(frac9[!is.na(frac9)])
pdf(sprintf("%s/%s_extreme.pdf", diag.dir, pref), height=4, width=8)
par(mfrow=c(1,2))
hist(frac9, main=pref, xlab="with extreme fractions")
...
pdf(sprintf("%s/%s_top_wrong.pdf", diag.dir, pref), height=4, width=8)
  ...
  for(exi in c(ex1, ex2, ex3, ex4)){
    ...
    plot(frac1s, fracN, main=rownames(ase1)[exi], bty="n", xlab="ASE1 fraction")
    kp = frac1s<=.5
    plot(density(na.omit(frac1s[kp]),from=0, to=.5), main=rownames(ase1)[exi], bty="n", xlim=0:1, ylim=c(0,4), xlab="ASE1 fraction")
    lines(density(na.omit(frac1s[!kp]),from=.5,to=1))
  }
dev.off()


#these rules are defined in specifications.txt  
#consider the sample-gene entry to be suspicious if whenever it has at least 10 ASE counts, 10% or more of them are hapN
#The whole gene if fraction of "suspicious" entries is >0.05
#The whole gene if fraction of "suspicious" entries and percent of samples with extreme ASE1 proportion is bigger then 20%
#Any particular "suspicious" sample-gene count
fracN = aseN/(ase1+ase2+aseN)>.1 & (ase1+ase2+aseN)>=10#(ase1+ase2-aseN)>5
fracNp = apply(fracN, 1, mean)
summary(fracNp>0.05)
min.samples = 10
summary(frac1ag[,2]>0.05  & frac1ag[,3]>min.samples)
summary(c(fracN))

aseo = ase1+ase2
#replace all the cases with too high fraction of bad ASE samples to zero ASE
#rm1 = fracNp>cut1; table(rm1)
rm1 = which(frac1ag[,2]>cut1 & frac1ag[,3]>min.samples); length(rm1)
rm1cnt = sum(aseo[rm1,]>0)
ase1[rm1,] = 0
ase2[rm1,] = 0
rm1cnt
  
#replace all the cases with too many extreme hap1 if fraction of bad ASE samples is at least 1%
aseo1 = ase1+ase2
fracE = apply(abs(ase1/(ase1+ase2)-0.5)>.4 & (ase1+ase2)>10, 1, mean)

#rm2 = fracNp>cut2 & fracE>cut3; table(rm2)
rm2 = which(frac1ag[,2]>cut2 & frac1ag[,1]>cut3); length(rm2)
rm2 = setdiff(rm2, rm1)
length(rm2)
rm2cnt = sum(aseo1[rm2,]>0)
rm2cnt
ase1[rm2,] = 0
ase2[rm2,] = 0
  
#replace all individual cases with suspicious fraction of aseN
aseo2 = ase1+ase2
fracN = aseN/(ase1+ase2+aseN)>.1 & (ase1+ase2+aseN)>10#(ase1+ase2-aseN)>5
ase1[fracN] = 0
ase2[fracN] = 0
rm3 = apply(fracN, 1, sum)>0;table(rm3)
rm3cnt = sum(aseo2[fracN]>0)
table(c(aseo!=(ase1+ase2)))
rm3cnt
  
filt = data.frame(cut1gns=sum(rm1), cut2gns=length(rm2), allrm=sum(aseo!=(ase1+ase2)), rm1cnt, rm2cnt, rm3cnt)
pref1 = sprintf("%s_%s", pref, model)
write.csv(filt, sprintf("%s/%s_filt.csv", diag.dir, pref1), quote=F, row.names=F)
filt
...
samout = sprintf("%s/sample.txt", wrk.dir)
