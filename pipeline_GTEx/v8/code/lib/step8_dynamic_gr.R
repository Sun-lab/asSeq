...
#/pine/scr/z/h/zhabotyn/R01/GTEx/v8/2020_06_20/plots/allTiss
bas.dir = "/pine/scr/z/h/zhabotyn/R01/GTEx/v8"
wrk.dir = "/pine/scr/z/h/zhabotyn/R01/GTEx/v8/2020_06_20"
...
#grj = 2
grs = c("allTiss", "Adipose", "Artery", "Blood", "Brain", "Esophagus", "Heart", "Skin")
dirs = 
list(
...
c("Artery_Aorta", "Artery_Coronary", "Artery_Tibial"),
c("Artery_Aorta", "Artery_Coronary", "Artery_Tibial","Whole_Blood"),
c("Brain_Caudate_basal_ganglia", "Brain_Cerebellar_Hemisphere", "Brain_Cortex", "Brain_Frontal_Cortex_BA9",
"Brain_Nucleus_accumbens_basal_ganglia"),
c("Esophagus_Gastroesophageal_Junction","Esophagus_Mucosa", "Esophagus_Muscularis"),
c("Heart_Atrial_Appendage", "Heart_Left_Ventricle"),
c("Skin_Not_Sun_Exposed_Suprapubic", "Skin_Sun_Exposed_Lower_leg")
)
#will used genes with eqtls significant at q-value 0.01
qsig = "01"

#dirs = c("BetaBin_nocov", "BetaBin_gPC", "BetaBin_gPC_PF")
dyns = c("nocov", "gPC", "gPC_PF")
j = 1
min.num.gns = 10
model = "long"
#model = "long"
for(grj in 1:length(grs)){
for(model in c("long", "short")){
  for(j in 1:3){
    dynj = dyns[j]
    #model = "long"
    cond = "ctcf"
    ...
    # --------------------------------------------------------------------
    tissi = 1
    qvala = as.character(0)
    for(tissi in 1:length(dirs[[grj]])){
      pref = dirs[[grj]][tissi]
      diri = sprintf("%s/%s/BetaBin", wrk.dir, pref)
      #Adipose_Visceral_Omentum_age_long_gPC_01.csv
      fili = sprintf("%s/%s_%s_%s_%s_%s.csv", diri, pref, cond, model, dynj, qsig)
      resi = fread(fili)
      resi = resi[!is.na(resi$pval),]
      dim(resi)
      colnames(resi)[1] = "id"
      resi[1:2,]
      
      
      pi0 = 2*mean(resi$pval > 0.5)
      qval = nrow(resi)*pi0*resi$pval/rank(resi$pval)
      qval = qvalue(resi$pval)$qvalue
      table(qval < 0.01)
      table(qval < 0.05)
      table(qval < 0.10)
      table(qval < 0.15)
      ...
      dim(resi)
    if(tissi==1){resA=resi}else{resA=rbind(resA, resi)}
    }
    resi = aggregate(resA$qval, by=list(resA$id, resA$chr, resA$strand, resA$start, resA$end,
      resA$promoter_start, resA$promoter_end, resA$start.e, resA$end.e), FUN=min)
    colnames(resi) = c("id", "chr", "strand", "start", "end",
        "promoter_start", "promoter_end", "start.e", "end.e", "qval")
    qval = resi$qval
    #resi[1:5,]
    ...
    out = sprintf("%s/%s_%s_%s_%s_dyn.csv", dyn.dir, grs[grj], cond, model, dynj)
    out
    write.csv(res3, out)
  
    #goseq
    for(qvali in 1:5){
      genes = as.integer(qval<qvals[qvali])
      if(sum(genes)>=min.num.gns){
      ...
      #  out = sprintf("%s/%s_%s_%s_goseq.csv", dyn.dir, pref, cond, model)
        out = sprintf("%s/%s_%s_%s_%s_goseq_qcut%s.csv", dyn.dir, grs[grj], cond, model, dynj, qvals[qvali])
        out
