...
for(model in c("long", "short")){
  for(j in 1:3){
    ...
    dist.fil = sprintf("%s/Reference/CTCF/CTCFBS_len_hist.pdf", bas.dir)
    if(!file.exists(dist.fil)){
      pdf(dist.fil, width=6, height=4)
    ...
    out = sprintf("%s/%s_%s_%s_%s_dyn.csv", dyn.dir, pref, cond, model, dynj)
    out
    write.csv(res3, out)
    ...
    for(qvali in 1:5){
      ...
      #  out = sprintf("%s/%s_%s_%s_goseq.csv", dyn.dir, pref, cond, model)
        out = sprintf("%s/%s_%s_%s_%s_goseq_qcut%s.csv.csv", dyn.dir, pref, cond, model, dynj, qvals[qvali])
        out
  ...
  #  out = sprintf("%s/%s_%s_%s_dyn.csv", dyn.dir, pref, cond, model)
    ...
    for(qvali in 1:5){
      ...
      if(sum(genes)>=min.num.gns){
        ...
        under = path[path[,"under_represented_pvalue"]<0.01,]
      #  out = sprintf("%s/%s_%s_%s_goseq.csv", dyn.dir, pref, cond, model)
        out = sprintf("%s/%s_%s_%s_%s_goseq_qcut%s.csv", dyn.dir, pref, cond, model, dynj, qvals[qvali])
        out
        write.csv(rbind(over, under), out, row.names=F)
      }
    }
