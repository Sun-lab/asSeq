...
#  kp = which(SNPInfo[,3]>=(ePos[geni]-local.distances[geni]) & SNPInfo[,3]<(ePos[geni]+local.distances[geni]));length(kp)
  ...
  if(kp>0){
    ...
    time1 = proc.time()
#    Xnew = X[,-c(65:63)]; dim(X); dim(Xnew)
    Xnew = X[,-c(63)]; message(nrow(X), " ", nrow(Xnew))
#    Xnew = X; dim(X); dim(Xnew)
    asSeq:::trecase(Y=cnts[,1,drop=F], Y1=cnts[,2,drop=F], Y2=cnts[,3,drop=F], X=Xnew, Z=t(genfi), output.tag = output.tagi, 
