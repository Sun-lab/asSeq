#sbatch -p general -t 03-00:00:00 -o step6_glm_nocov_1.out --mem=4g --wrap="R CMD BATCH '--args 1' step6_glm_nocov.R step6_nocov_1.Rout"
...
#i = i + 1
        ...
        attempt = tryCatch({
          ...
          fit1 = fit0 <- vglm(cbind(yL, nL-yL) ~ 1, family=betabinomial, data=phenoL,
              trace = FALSE, subset = nL > 0)

          ini = c(fit1@coefficients, 0); ini[2][ini[2]< -5] = -5
          ini[1][ini[1]< -5] = -5
          ini[1][ini[1]> 5] = 5
          fit <- vglm(cbind(yL, nL-yL) ~ 1 + cnd, family=betabinomial, data=phenoL,
...
#          summary(fit)
        ...
        fit.bin1 <- glm(cbind(yL, nL-yL) ~ 1, family=binomial, data=phenoL)
        fit.bin <- glm(cbind(yL, nL-yL) ~ 1 + cnd, family=binomial, data=phenoL)
        fit.qua <- glm(cbind(yL, nL-yL) ~ 1 + cnd, family=quasibinomial, data=phenoL)
        #fit
        #logLik(fit.bin)
        #resL1.bb[i,]
        ...
        attempt = tryCatch({
          ...
          fit1 = fit0 <- vglm(cbind(yS, nS-yS) ~ 1, family=betabinomial, data=phenoS,
              trace = FALSE, subset = nS > 0)

          ini = c(fit1@coefficients, 0); ini[2][ini[2]< -5] = -5
          ini[1][ini[1]< -5] = -5
          ini[1][ini[1]> 5] = 5
          fit <- vglm(cbind(yS, nS-yS) ~ 1 + cnd, family=betabinomial, data=phenoS,
        ...
        fit.bin1 <- glm(cbind(yS, nS-yS) ~ 1, family=binomial, data=phenoS)
        fit.bin <- glm(cbind(yS, nS-yS) ~ 1 + cnd, family=binomial, data=phenoS)
        fit.qua <- glm(cbind(yS, nS-yS) ~ 1 + cnd, family=quasibinomial, data=phenoS)
        #fit
        #logLik(fit.bin)
        #resS1.bb[i,]
...
write.csv(resL1.bb, sprintf("%s/%s_long_vglm_nocov.csv", out.dir, mod), quote=F)
write.csv(resS1.bb, sprintf("%s/%s_short_vglm_nocov.csv", out.dir, mod), quote=F)

write.csv(resL1.qp, sprintf("%s/%s_long_glmqp_nocov.csv", out.dir, mod), quote=F)
write.csv(resS1.qp, sprintf("%s/%s_short_glmqp_nocov.csv", out.dir, mod), quote=F)

write.csv(resL1.bin, sprintf("%s/%s_long_glmbin_nocov.csv", out.dir, mod), quote=F)
write.csv(resS1.bin, sprintf("%s/%s_short_glmbin_nocov.csv", out.dir, mod), quote=F)
