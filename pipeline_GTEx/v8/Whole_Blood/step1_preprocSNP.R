...
#  sum(geno[i,]==-1)==0
#}
#no.nas0 = sapply(1:nrow(geno), fun, geno=geno)

w.nas = aggregate(x4u[,3]==-1, by=list(x4u[,1]), FUN=sum)
w.nas = w.nas[w.nas[,2]>0,1]
#no.nas = which(apply(geno==-1, 1, sum)>0); 
#length(no.nas)
if(length(w.nas)>0){
  geno = geno[-w.nas,]
  SNPInfo = SNPInfo[-w.nas,]
}
dim(SNPInfo)
dim(geno)
...
for(nsub in numsam){
  ...
  #MAFo = apply(genos[1:100,], 1, function(x) (sum(x %in% c(1,2)) + 2*sum(x==3)) /(2*length(x)))
  MAF = get_mafs(genos) 
  #summary(MAF)
